...
public class ClassUtils {
	...
	public static void declaredFields(Class clazz, Map<Integer, Field> sortedAllFiledMap,
		...
		int index = 0;
		for (Entry<Integer, Field> entry : fieldCache.getSortedAllFiledMap().entrySet()) {
	...
	private static void declaredFields(Class clazz, Boolean convertAllFiled) {
		...
		if (indexFiledMap.isEmpty()) {
			// 清空
			orderFiledMap.clear();
			for (Field field : tempFieldList) {
				declaredCusOneField(field, indexFiledMap, ignoreMap, convertAllFiled);
			}
			// 重置字段排序
			if (!indexFiledMap.isEmpty()) {
				Map<Integer, Field> tempIndexFiledMap = new TreeMap<Integer, Field>();
				int i = 0;
				for (Entry<Integer, Field> entry : indexFiledMap.entrySet()) {
					tempIndexFiledMap.put(i, entry.getValue());
					i++;
				}
				indexFiledMap.clear();
				indexFiledMap.putAll(tempIndexFiledMap);
			}
		}
		...
		for (List<Field> fieldList : orderFiledMap.values()) {
			for (Field field : fieldList) {
				while (tempIndexFiledMap.containsKey(index)) {
					sortedAllFiledMap.put(index, tempIndexFiledMap.remove(index));
					index++;
				}
				sortedAllFiledMap.put(index, field);
				index++;
	 ...
	 */
	private static void declaredCusOneField(Field field, Map<Integer, Field> indexFiledMap,
		...
		ApiModelProperty excelProperty = field.getAnnotation(ApiModelProperty.class);
		if (excelProperty != null && !excelProperty.hidden() && excelProperty.position() >= 0) {
			if (excelProperty.hidden()) {
				ignoreMap.put(field.getName(), field);
			}

			if (indexFiledMap.containsKey(excelProperty.position())) {
				throw new ExcelCommonException("The index of '" + indexFiledMap.get(excelProperty.position()).getName()
						+ "' and '" + field.getName() + "' must be inconsistent");
			}
			indexFiledMap.put(excelProperty.position(), field);
			return;
		}
